...
namespace
{
    ...
    void Apply(const char* key, const char* val)
    {
        ...
        else if (KeyIs(key, "swipe_confirm_frames")) g_cfg.swipeConfirmFrames = atoi(val);
        else if (KeyIs(key, "hold_enter_fx"))      g_cfg.holdEnterFx       = (float)atof(val);
        else if (KeyIs(key, "hold_exit_fx"))       g_cfg.holdExitFx        = (float)atof(val);
        else if (KeyIs(key, "hold_drop_fy"))       g_cfg.holdDropFy        = (float)atof(val);
        else if (KeyIs(key, "hold_exit_frames"))   g_cfg.holdExitFrames    = atoi(val);
        else if (KeyIs(key, "first_repeat_ms"))    g_cfg.firstRepeatMs     = atoi(val);
        else if (KeyIs(key, "min_repeat_ms"))      g_cfg.minRepeatMs       = atoi(val);
        ...
        else if (KeyIs(key, "recenter_fx"))        g_cfg.recenterFx        = (float)atof(val);
        else if (KeyIs(key, "rearm_timeout_ms"))   g_cfg.reArmTimeoutMs    = atoi(val);
        else if (KeyIs(key, "arm_center_fx"))      g_cfg.armCenterFx       = (float)atof(val);
